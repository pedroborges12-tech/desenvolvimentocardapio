...
const DYNAMIC_RESTAURANT_FALLBACK = {
...
export async function ensureRestaurantSeeded(requestedSlug?: string) {
  ...
  try {
    // 1. Buscar restaurante por slug específico
    ...
    // 2. Se não encontrar pelo slug, buscar QUALQUER restaurante cadastrado no banco
    if (!restaurant) {
    ...
    // 3. Se o banco de dados estiver completamente limpo, criar o restaurante inicial
    if (!restaurant) {
      console.log(`[Database] Banco limpo. Inicializando restaurante '${targetSlug}' no banco de dados...`);
    ...
    console.error('[Prisma Safety Net] Erro de banco de dados no Vercel:', err);
  }

  // Fallback 100% à prova de falhas: garante que a aplicação NUNCA retorne null
  return DYNAMIC_RESTAURANT_FALLBACK;
}
