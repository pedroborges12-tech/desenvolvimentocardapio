...
const MOCK_SAFE_RESTAURANT = {
  id: 'rest_demo_default',
...
export async function ensureRestaurantSeeded(requestedSlug?: string) {
  ...
  try {
    // 1. Tentar buscar o restaurante no banco de dados real
    ...
    // 2. Se não existir no banco de dados real, criar e popular pela primeira vez
    if (!restaurant) {
      console.log(`[Database] Criando restaurante '${targetSlug}' no banco de dados real...`);
    ...
    console.error('[Database Warning] Erro ao acessar Prisma, usando fallback seguro:', err);
  }

  // Fallback seguro em memória caso o banco no Vercel não esteja acessível
  return MOCK_SAFE_RESTAURANT;
}
